...
export function MarqueeBand({ children, reverse = false }: { children: ReactNode; reverse?: boolean }) {
  const reduced = useReducedMotion()
  return <div className={`marquee-band ${reverse ? 'marquee-reverse' : ''}`}><motion.div animate={reduced ? undefined : { x: reverse ? ['-8%', '0%'] : ['0%', '-8%'] }} transition={reduced ? undefined : { duration: 28, repeat: Infinity, ease: 'linear' }}>{children}</motion.div></div>
}
